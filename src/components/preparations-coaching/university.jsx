"use client"

const offerings = [
  {
    id: 1,
    title: "Exam-Focused Coaching",
    description: "We offer structured programs to prepare students for:",
    points: [
      "FMGE (Foreign Medical Graduate Examination)",
      "NExT (National Exit Test)",
      "International licensing exams (USMLE, PLAB, DHA, etc.)",
      "Our coaching combines theory, practice tests, and doubt-clearing sessions to maximize performance.",
    ],
  },
  {
    id: 2,
    title: "Study Materials & Resources",
    description: "Students receive access to high-quality learning materials:",
    points: [
      "Comprehensive notes and study guides",
      "Practice question banks and mock tests",
      "Online lectures and video tutorials",
      "These resources allow students to study effectively and reinforce exam readiness.",
    ],
  },
  {
    id: 3,
    title: "Mentorship & Personalized Guidance",
    description: "Our expert faculty and doctors provide:",
    points: [
      "One-on-one mentoring for weak areas",
      "Strategies for time management and exam techniques",
      "Continuous support to boost confidence and reduce exam stress",
    ],
  },
  {
    id: 4,
    title: "Benefits of Preparations & Coaching",
    description: "",
    points: [
      "Achieve top scores in FMGE, NExT, and other exams",
      "Gain confidence with practice tests and expert guidance",
      "Strengthen theoretical knowledge and practical understanding",
      "Receive mentorship to plan future career and specialization",
    ],
  },
  {
    id: 5,
    title: "Related Services You Might Be Interested In",
    description: "",
    points: [
      "Counselling Services – Career guidance and destination planning",
      "Admissions Assistance – Admission support for top medical universities",
      "Clinical Trainings – Hands-on practical exposure",
      "Learning Hub – Digital learning materials, webinars, and workshops",
      "Internships – Real-world experience in hospitals and universities",
    ],
  },
];




export default function ServicesOfferings() {
  return (
    <section className="py-16 md:py-24 px-4 md:px-8">
      <div className="max-w-7xl mx-auto space-y-16 md:space-y-24 px-6">
        {offerings.map((offering, index) => {
          const isEven = index % 2 === 0

          return (
            <div key={offering.id} className="flex flex-col md:flex-row gap-8 md:gap-12 items-center">
              <div className={`flex-1 ${isEven ? "md:order-1" : "md:order-2"}`}>
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-lg border-2 border-primary flex items-center justify-center">
                      <span className="text-primary font-bold text-lg">{offering.id}</span>
                    </div>
                    <h3 className="text-2xl md:text-3xl font-bold text-foreground">{offering.title}</h3>
                  </div>

                  <p className="text-foreground/70 text-lg leading-relaxed">{offering.description}</p>

                  {/* <p className="text-foreground/60 text-base">{offering.fullDescription}</p> */}

                  <ul className="space-y-3 pt-4">
                    {offering.points.map((point, idx) => (
                      <li key={idx} className="flex gap-3 items-start">
                        <span className="text-primary font-bold flex-shrink-0 mt-1">✓</span>
                        <span className="text-foreground/80">{point}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              </div>

              <div className={`flex-1 ${isEven ? "md:order-2" : "md:order-1"}`}>
                <div
                  className={`h-64 md:h-80 rounded-lg border-2 flex items-center justify-center ${
                    isEven ? "border-primary/30 bg-primary/5" : "border-secondary/30 bg-secondary/5"
                  }`}
                >
                  <div className="text-center">
                    <div className={`text-6xl font-bold mb-2 ${isEven ? "text-primary/20" : "text-secondary/20"}`}>
                      {String(offering.id).padStart(2, "0")}
                    </div>
                    <p className="text-foreground/40 text-sm">{offering.title}</p>
                  </div>
                </div>
              </div>
            </div>
          )
        })}
      </div>
    </section>
  )
}
